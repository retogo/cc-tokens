...
- **`pricing.ts`**: コスト換算。**Claude Code 本体の `/cost` と同じ式**に揃えてある: cache write は `cache_creation.ephemeral_1h_input_tokens` 分を **input×2**、残りを **input×1.25**（1h 内訳は総量で頭打ち）、cache read は input×0.1。`usage.speed === "fast"` は opus 系のみ専用の価格行（Opus 5 / 4.8 は 10/50、4.7 / 4.6 は 30/150）、`usage.inference_geo === "us"` はトークン費用のみ ×1.1、web search は $0.01/リクエストで geo 倍率の外側に足す。単価に効くこれらの属性は `TokenUsage` ではなく `TurnRecord.pricing`（`PricingAttributes`）に置く: `TokenUsage` は daemon の JSON スキーマに出るので、課金内部値でそれを広げない。
...
- **`resets_at` が過去になった瞬間（リセット境界を跨いで再取得が済むまで）: `[resets_at, now]`**。その時刻に新しいウィンドウが始まっているので、過去 reset を捨てて `[now - 5h, now]` に退避すると**リセット済みの消費が使用量・内訳（BY MODEL / BY SESSION）に残って見える**。5h より古い値は信用せず直近 5h にクランプする。% / reset 時刻は stale なので出さない。
- どちらも無い瞬間（起動直後 / 401 / 429 / ネット断）: `[now - 5h, now]`、% / reset 時刻は**表示しない**（ローカル近似で埋めない）。これは独立モードではなく一時的な degraded 表示で、使用量・バーン・内訳は出し続ける。
