...
**真の % とリセット時刻の表示・limit の逆算にのみ**使い、それ以外はトランスクリプトからローカルで算出する。
API は常時取得する。取得できない時（トークン無し/期限切れ/レート制限/オフライン）は % とリセット時刻を隠し、使用量・バーン・内訳は出し続ける。
...
# 単発レポート（% / リセット時刻 / limit 推定は API が取れた時に含まれる）
...
- **% と リセット時刻**: `/api/oauth/usage`（常時取得）から取得できた時のみ表示。
...
- [API 連携と 5h ウィンドウ](./api-and-window.ja.md) — `usage`、OAuth トークン解決、401 / 429 の扱い、ウィンドウ定義。
...
- official API が使えない時（トークン無し/期限切れ/レート制限/オフライン）は % とリセット時刻を表示しない（使用量・バーン・内訳は出る）。
